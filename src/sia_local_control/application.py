...
from .app_ui import SiaLocalControlUI
from .app_state import SiaLocalControlState

log = logging.getLogger()

class SiaLocalControlApplication(Application):
    config: SiaLocalControlConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: SiaLocalControlUI = None
        self.state: SiaLocalControlState = None

    async def setup(self):
        self.ui = SiaLocalControlUI()
        self.state = SiaLocalControlState()
